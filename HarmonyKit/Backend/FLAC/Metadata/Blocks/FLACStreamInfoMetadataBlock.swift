...
struct FLACStreamInfoMetadataBlock {
    ...
    let minimumBlockSize: UInt32
    let maximumBlockSize: UInt32
    ...
    init(bytes: Data, header: FLACMetadataBlockHeader) {
        ...
        minimumBlockSize = advancedBytes[0..<2].withUnsafeBytes { $0.load(as: UInt32.self) }
        advancedBytes = advancedBytes.advanced(by: 2)

        maximumBlockSize = advancedBytes[0..<2].withUnsafeBytes { $0.load(as: UInt32.self) }
        advancedBytes = advancedBytes.advanced(by: 2)

        minimumFrameSize = advancedBytes[0..<3].withUnsafeBytes { $0.load(as: UInt32.self) }
        advancedBytes = advancedBytes.advanced(by: 3)

        maximumFrameSize = advancedBytes[0..<3].withUnsafeBytes { $0.load(as: UInt32.self) }
        advancedBytes = advancedBytes.advanced(by: 3)
        ...
        let a = advancedBytes[0..<2].withUnsafeBytes { $0.load(as: UInt32.self) } << 4
        ...
        totalSamples = (UInt64(advancedBytes[3]) & 0x0F) << 32 |
                        UInt64(advancedBytes[4..<8].withUnsafeBytes { $0.load(as: UInt64.self) })
